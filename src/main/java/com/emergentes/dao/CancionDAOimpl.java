...
public class CancionDAOimpl extends ConexionDB implements CancionDAO {
    ...
    public void insert(Cancion cancion) throws Exception {
        try {
            this.conectar();
<<<<<<< HEAD
            String sql = "INSERT INTO cancion (titulo, duracion, fecha, artista_id, album_id, genero_id, grupo_id, cancion_img, cancion_lk) VALUES (?, ?, ?, ?, ?, ?, ?, ?,?)";
=======
...
>>>>>>> e435e9c8968e6d4377bfdb1107cd2e58beb972b6
            ...
            ps.setString(9, cancion.getCancion_lk());
            ps.executeUpdate();
    ...
    public void update(Cancion cancion) throws Exception {
        try {
            this.conectar();
            String sql = "UPDATE cancion SET titulo = ?, duracion = ?, fecha = ?, artista_id = ?, album_id = ?, genero_id = ?, grupo_id = ?, cancion_img = ?, cancion_lk = ? "
                    + "WHERE cancion_id = ?";
            PreparedStatement ps = this.conn.prepareStatement(sql);
            ps.setString(1, cancion.getTitulo());
            ps.setString(2, cancion.getDuracion());
            ...
            ps.setString(8, cancion.getCancion_img());
            ps.setString(9, cancion.getCancion_lk());
            ps.setInt(10, cancion.getCancion_id());
            ps.executeUpdate();
        } catch (Exception e) {
            throw e;
        } finally {
...
<<<<<<< HEAD
                cancion.setCancion_lk(rs.getString("cancion_lk"));
=======
>>>>>>> e435e9c8968e6d4377bfdb1107cd2e58beb972b6
            }
        } catch (Exception e) {
